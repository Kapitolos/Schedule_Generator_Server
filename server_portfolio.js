...
app.get('/staff-availability', (req, res) => {
  getJsonFromS3('staff_availability_portfolio.json', (err, data) => {
    if (err) {
...
app.post('/update-booked-dates', (req, res) => {
    ...
    const key = 'staff_availability_portfolio.json';

    uploadJsonToS3(key, updatedData, (err) => {
        if (err) {
            console.error('Error writing to staff_availability_portfolio.json:', err);
            res.status(500).send('Error writing to file');
            return;
        }
        console.log('Successfully updated staff_availability_portfolio.json');
...
router.post('/update-staff-availability', (req, res) => {
  ...
  getJsonFromS3('staff_availability_portfolio.json', (err, currentData) => {
    if (err) {
      console.error('Error reading staff_availability_portfolio.json:', err);
      return res.status(500).send('Error reading file');
    }
    ...
    uploadJsonToS3('staff_availability_portfolio.json', currentData, (err) => {
      if (err) {
        console.error('Error writing to staff_availability_portfolio.json:', err);
...
router.post('/restricted-slots', (req, res) => {
    ...
    const key = 'restricted_slots_portfolio.json';
...
app.get('/restricted-slots', (req, res) => {
    const key = 'restricted_slots_portfolio.json'; // Ensure this matches your S3 file key
...
app.post('/add-staff', (req, res) => {
    ...
    getJsonFromS3('staff_availability_portfolio.json', (err, staffAvailability) => {
        if (err) {
            console.error('Error reading staff_availability.json:', err);
        ...
        uploadJsonToS3('staff_availability_portfolio.json', staffAvailability, (err) => {
            if (err) {
                console.error('Error writing to staff_availability_portfolio.json:', err);
                res.status(500).send('Error writing to file');
                return;
            }
            console.log('Successfully added new staff member to staff_availability_portfolio.json');
